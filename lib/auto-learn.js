...
const { isValidConvId, MEMORY_CATEGORIES } = require("./validators");
...
function tryAcquireCooldown(convId) {
  // 严格校验 convId 格式（10-16 位纯数字），防止非法 ID 绕过冷却期
  if (!isValidConvId(convId)) return false;
  const now = Date.now();
...
function parseAutoLearnOutput(output) {
  ...
  if (results.length > MAX_OPS_PER_CALL) {
    ...
    results = results.slice(0, MAX_OPS_PER_CALL);
  }

  if (results.length > 0) {
    const counts = { add: 0, update: 0, delete: 0 };
    for (const r of results) counts[r.op] = (counts[r.op] || 0) + 1;
    console.log(`[auto-learn] parsed ${results.length} ops (add: ${counts.add}, update: ${counts.update}, delete: ${counts.delete})`);
  }
