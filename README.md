...
从 ChatGPT 官网导出的 `conversations.json` 可以一键导入。打开设置面板的「导入与总结」Tab：

1. **上传 JSON 文件** — 拖拽或点击选择，大文件也不卡（后台 Worker 解析）
2. **勾选对话** — 可以只导入/总结部分对话，也可以不导入直接总结现有本地对话
3. **AI 分析** — 点击「总结选中对话」，AI 提取你的偏好、风格、身份等新发现
4. **融合到现有 Prompt** — 审核发现内容后点击「融合」，AI 在你现有 Prompt 基础上智能合并，不会覆盖你调好的内容
5. **预览确认** — 融合结果可编辑，应用前自动备份旧 Prompt
